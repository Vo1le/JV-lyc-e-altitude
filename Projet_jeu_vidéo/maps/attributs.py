...
def main():
    ...
    indexname = list(newimages.keys())
    for i in range(0,len(newimages)):
        local = ast.literal_eval(newimages[indexname[0]])
        tile_maps[indexname[i]] = {"tile_size": 32, "attributs": [create_tile_atlas(0, 0, local[0], local[1])]}
